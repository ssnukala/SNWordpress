...
add_action( 'init', 'retina_add_image_sizes' );
function retina_add_image_sizes() {
	add_image_size( 'retina-image-featured', 200, 200, true );
}

/** Retina Get Image Id Manual */
function retina_get_image_id_manual( $num = 0 ) {
...
/** Retina Get Image Id Auto */
function retina_get_image_id_auto( $num = 0 ) {
	
	/** Global Object */
	global $post;
	
	/** Manual Check At Priority */
	$id = retina_get_image_id_manual( $num );
...
function retina_get_image( $args = array() ) {
	...
	$id = '';
	if( $args['mode'] == 'manual' ) {		
		$id = retina_get_image_id_manual( $args['num'] );			
	} else {	
		$id = retina_get_image_id_auto( $args['num'] );	
	}
