<div id="comments">
  ...
  <p class="nopassword"><?php _e( 'This post is password protected. Enter the password to view any comments.', 'retina' ); ?></p>
  </div><!-- #comments -->
  <?php
  /**
  ...
  <h3 id="comments-title">
    <?php printf( _n( 'One Thought on &ldquo;%2$s&rdquo;', '%1$s Thoughts on &ldquo;%2$s&rdquo;', get_comments_number(), 'retina' ), get_comments_number(), '<span>' . get_the_title() . '</span>' ); ?>
  </h3>
  ...
  <div id="comments-nav-below" class="clearfix">
    ...
    <?php
    ob_start();
	previous_comments_link( __( '&larr; Older Comments', 'retina' ) );
	$previous_comments_link = ob_get_clean();
	
	ob_start();
	next_comments_link( __( 'Newer Comments &rarr;', 'retina' ) );
    ...
    <h3 class="assistive-text"><?php _e( 'Comment navigation', 'retina' ); ?></h3>
  ...
  ?>
  <p class="nocomments"><?php _e( 'Comments are closed.', 'retina' ); ?></p>
  <?php endif; ?>
